...
<section class="month" id="month-{number}">
  ...
  <div class="set-heading">
    ...
    <h3>Ten questions that test the picture, not just the arithmetic</h3>
  </div>
...
month_1 = [
    ...
    (
        ...
        Why do its coordinates change even though the arrow does not?""",
        r"""<p>We need
        ...
        relative to a chosen set of measuring arrows. Changing the basis changes the
        instructions, not the geometric vector. It is like describing the same
    ...
    (
        ...
        <p>A displacement does not change. If both endpoints have the same translation
        subtracted, it cancels in \(Q-P\). Position vectors depend on an origin;
        displacement vectors do not. Confusing these is a common source of otherwise
        elegant wrong answers.</p>""",
    ),
    (
        ...
        third would be \(5\), not \(4\). So \(\mathbf z\) is not in the span.
    ...
    (
        ...
        <p>Components do not remove units. A velocity could first be multiplied by a
        time interval to produce a displacement; only then could the vectors be added.
        Dimensional consistency is a test of meaning, not merely of arithmetic.</p>""",
    ),
]
...
month_2 = [
    ...
    (
        ...
        What does the value mean, and what does it <em>not</em> mean?""",
        ...
        \(0\) means perpendicular, and \(-1\) means opposite direction. It does not
        mean that “60% of the vector is present”. The actual component of
        \(\mathbf u\) along \(\mathbf v\) has length \(\|\mathbf u\|\cos\theta=3\).</p>""",
    ),
    (
    ...
    (
        ...
        \(\mathbf u\cdot\mathbf v=0\). Use
        \(\|\mathbf u-\mathbf v\|^2\) to explain why the vectors form a right angle,
        rather than merely quoting the angle formula.""",
        r"""<p>Expand:
        \[
        \|\mathbf u-\mathbf v\|^2
    ...
    (
        ...
        <p>This sequence shows why work uses a dot product. It needs the signed
        along-component, not a perpendicular direction or an area.</p>""",
    ),
    (
...
month_3 = [
    ...
    (
        ...
        <p>Torque is a cross product because turning depends on the part of the force
        perpendicular to the lever arm, not the part along it.</p>""",
    ),
    (
    ...
    (
        ...
        changes build the curved motion. The cross product is the local generator of
        rotation, not the entire finite rotation in one step.</p>""",
    ),
    (
        ...
        the same normal line. A plane needs a normal direction, not a prescribed
        normal length. The sign merely selects one of the two orientations.</p>""",
    ),
    (
...
month_4 = [
    (
        ...
        \(c_1\mathbf i+c_2\mathbf j+c_3\mathbf k\). The array is a compact vector-valued
        mnemonic for obtaining the three components, not an ordinary determinant
        whose entries are all scalars.</p>""",
    ),
    (
        r"""Expand
        \[
    ...
    (
        ...
        r"""<p>It is a mnemonic for cofactor expansion, not an ordinary numerical
        determinant. Expanding produces the three scalar components multiplying the
        basis vectors.</p>
    ...
    (
        ...
        <p>The parallelepiped's ordinary volume does not change, because volume uses
        the absolute value. What changes is orientation: the ordering has switched
        handedness. The sign records that orientation while the absolute value records
        ordinary volume.</p>""",
    ),
    (
...
<p>Start with a distinction that quietly controls everything else. A vector is not
the column of numbers on the page. It is the directed change those numbers describe.
The same arrow may begin anywhere, and the same arrow may acquire different coordinates
when the measuring axes change.</p>
<p>The standard basis vectors \(\mathbf i,\mathbf j,\mathbf k\) are three unit arrows,
not decoration. Writing \(3\mathbf i-2\mathbf j+\mathbf k\) means: assemble an arrow
from three independent directional moves. This is why basis notation is useful in
physics: every coefficient answers “how much in this measuring direction?”</p>
...
Even there it is not, by itself, a finite rotation through a large angle. The tangent
arrow is the local story; a rotation matrix accumulates that story around the
curve.</p>"""

lens_4 = r"""
<h3>The determinant is not a trick for remembering three components</h3>
...
collapses to zero. These are not coincidences; they are the cross product's defining
behaviours.</p>
...
is signed volume.</p>
<p>Use the determinant to calculate, but use dot products to check. A claimed cross
product that is not perpendicular to both inputs is wrong, however tidy its expansion
looks.</p>"""

lens_5 = r"""
<h3>Choose an operation by the question it answers</h3>
<p>“There are vectors” is not enough information to choose a formula. Ask what the
problem wants. A scalar measuring along-ness suggests dot. A vector normal to a plane
suggests cross. An actual along-component suggests projection. Successive motions
suggest addition.</p>
...
<text x="380" y="318" class="svg-strong" text-anchor="middle">Choose from the meaning of the output, not from the presence of arrows.</text>
</svg>
</figure>"""
...
<header><div class="hero">
  ...
  determinants, and physical decisions. The aim is not to become faster at a recipe.
  It is to know what each operation is measuring before you calculate it.</p>
...
<section class="how">
  ...
  <p>One set is one month's recurring problem, not one evening's worksheet. Begin each
  question with a sketch and one sentence naming the quantity you expect: a scalar,
  an along-component, a normal vector, an area, or a turning direction.</p>
